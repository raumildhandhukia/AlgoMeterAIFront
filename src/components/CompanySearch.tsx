...
const CompanySearch: React.FC = () => {
  ...
  const [companyTagsData, setCompanyTagsData] = useState<
    Record<string, CompanyTagsResponse>
  >({});
  const [loadingCompanyTags, setLoadingCompanyTags] = useState<
    Record<string, boolean>
  >({});
  const [showDetailedTags, setShowDetailedTags] = useState<
    Record<string, boolean>
  >({});
  const [showCompanyFrequency, setShowCompanyFrequency] = useState<
    Record<string, boolean>
  >({});
  ...
  // Initial data fetch when component mounts - fetch all companies' data
  useEffect(() => {
    if (!rateLimitInfo.isRateLimited) {
      fetchQuestionsData([], 1, currentFilters);
    }
  }, []);
  ...
  const getFilteredCompanies = (): Company[] => {
    ...
    return fuse
      .search(searchTerm)
      .slice(0, 10)
      .map((result: FuseResult<Company>) => result.item);
  };
  ...
  const fetchQuestionsData = async (
    ...
    // When no company is selected, we still want to fetch all companies' data
    ...
    try {
      ...
      if (response.statusCode === 429 || response.isRateLimited) {
        ...
        setRateLimitInfo({
          ...
          }. Because of limited resources, please wait ${resetTimeFormatted} seconds before trying again.`,
        });
  ...
  const handleCompanySelect = (company: Company) => {
    ...
    if (newSelectedCompanies.includes(company.slug)) {
      newSelectedCompanies.splice(
        newSelectedCompanies.indexOf(company.slug),
        1
      );
    } else {
  ...
  const handlePageChange = (page: number) => {
    if (page === currentPage || isLoading || rateLimitInfo.isRateLimited)
      return;
  ...
  const handleFilterChange = (
    ...
    // Always fetch data, even when no companies are selected
    fetchQuestionsData(
      companiesForFetch,
      ...
      <div className="text-center py-12 bg-gray-800/50 rounded-lg border border-gray-700">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-400 mb-6">
            We couldn't find any questions matching your criteria. Try adjusting
            your filters or search for a different company.
          </p>
          <button
            onClick={() =>
              handleFilterChange({
  ...
  const handleGetDetailedFrequency = (titleSlug: string): void => {
    ...
    setShowDetailedTags((prev: Record<string, boolean>) => ({
      ...prev,
      [titleSlug]: true,
    }));
  };
  ...
  const fetchCompanyTags = async (titleSlug: string): Promise<void> => {
    ...
    setLoadingCompanyTags((prev: Record<string, boolean>) => ({
      ...prev,
      [titleSlug]: true,
    }));
      ...
      {rateLimitInfo.isRateLimited && (
        <div className="bg-red-900/50 border border-red-700 text-white p-4 rounded-lg mb-6 text-center">
          ...
          <p className="text-sm mb-2">
            Because of limited resources, we need to limit requests.
          </p>
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold">
              {selectedCompanies.length > 0
                ? `Questions from ${selectedCompanies.length} companies (${
                    pagination?.total || 0
                  } questions)`
                : `All company questions (${pagination?.total || 0} questions)`}
            </h2>
            <div className="text-gray-400">
              Showing{" "}
              {((pagination?.page || 1) - 1) * (pagination?.limit || 0) + 1} to{" "}
              {Math.min(
          ...
          {pagination && pagination.totalPages > 1 && (
            <div className="flex justify-center mb-6 space-x-2">
              <button
                ...
                disabled={
                  !pagination.hasPrevPage || rateLimitInfo.isRateLimited
                }
              ...
              <button
                ...
                disabled={
                  !pagination.hasNextPage || rateLimitInfo.isRateLimited
                }
                className={`px-4 py-2 rounded-md ${
                  !pagination.hasNextPage || rateLimitInfo.isRateLimited
                    ? "bg-gray-800 text-gray-500 cursor-not-allowed"
            ...
            {questions.map((question) => (
              ...
              >
                <div className="flex items-center space-x-2 mb-2">
                  <span
                    className={`px-2 py-0.5 text-xs font-medium rounded-md ${getDifficultyColor(
                      question.difficulty
                    )}`}
                  >
                ...
                <div className="flex flex-wrap items-center gap-2 mt-3 -mx-2">
                  <button
                    onClick={() =>
                      setShowCompanyFrequency({
                        ...showCompanyFrequency,
                        [question.titleSlug]:
                          !showCompanyFrequency[question.titleSlug],
                      })
                    }
                    className="px-3 py-1 bg-yellow-800 hover:bg-blue-700 text-white text-xs rounded-md transition-colors ml-2"
                  >
                  ...
                  <div className="mt-4 bg-gray-750 p-3 rounded-md border border-gray-700">
                    <div className="flex flex-wrap gap-1">
                      {question.companies && question.companies.length > 0 ? (
                        question.companies.map((companyTag, idx) => (
                          <span
                            key={`${companyTag.slug}-${idx}`}
                            className="inline-flex items-center px-2 py-0.5 text-xs font-medium bg-gray-700 text-gray-300 rounded-md"
                          >
                            {companyTag.company}
                            <span
                              className={`ml-1 ${getFrequencyColor(
                                companyTag.frequency
                              )}`}
                            >
                              ({companyTag.frequency})
                            </span>
                ...
                {showDetailedTags[question.titleSlug] && (
                  <div className="mt-4 bg-gray-750 p-3 rounded-md border border-gray-700">
                    ...
                    {loadingCompanyTags[question.titleSlug] && (
                      <div className="flex justify-center items-center py-8">
                        ...
                        <span className="ml-2 text-gray-300">
                          Loading company data... It might take upto 30 seconds.
                          Please be patient.
                        </span>
                      </div>
                    )}
                    ...
                    {!loadingCompanyTags[question.titleSlug] &&
                      !companyTagsData[question.titleSlug] && (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          ...
                          <div className="bg-gray-800 p-3 rounded-md">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto">
                              {companyTagsData[question.titleSlug]?.three_months
                                ?.length > 0 ? (
                                companyTagsData[
                                  question.titleSlug
                                ].three_months.map((tag, idx) => (
                                  <div
                                    key={tag.slug || `three-month-${idx}`}
                          ...
                          <div className="bg-gray-800 p-3 rounded-md">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto">
                              {companyTagsData[question.titleSlug]?.six_months
                                ?.length > 0 ? (
                                companyTagsData[
                                  question.titleSlug
                                ].six_months.map((tag, idx) => (
                                  <div
                                    key={tag.slug || `six-month-${idx}`}
                                    className="flex justify-between items-center text-xs"
                                  >
                                    <span className="text-gray-300">
                                      {tag.name}
                          ...
                          <div className="bg-gray-800 p-3 rounded-md">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto">
                              {companyTagsData[question.titleSlug]
                                ?.more_than_six_months?.length > 0 ? (
                                companyTagsData[
                                  question.titleSlug
                                ].more_than_six_months.map((tag, idx) => (
                                  <div
                                    key={tag.slug || `more-than-six-${idx}`}
                                    className="flex justify-between items-center text-xs"
                                  >
                                    <span className="text-gray-300">
                                      {tag.name}
